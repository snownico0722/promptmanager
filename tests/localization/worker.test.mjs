...
const core = fs.readFileSync(new URL('../../src/i18n-core.js', import.meta.url), 'utf8');
// Execute the real worker, supplying browser/storage imports through the harness.
const worker = fs.readFileSync(new URL('../../src/service-worker.js', import.meta.url), 'utf8').replace(/^import[\s\S]*?;\n/gm, '');
...
function setup({ prompts = [{ uuid: 'a', title: 'Settings', content: 'Copy' }, { uuid: 'b', title: '', content: 'B' }], getPrompts } = {}) {
  ...
  const context = vm.createContext({
    ...
    chrome, setTimeout, clearTimeout, URL,
...
test('language changes during a delayed read settle to the newest language without duplicate IDs', async () => {
  ...
  await r.context.OPMI18n.setLanguage('en'); finish([]); await r.context.drainMenu();
