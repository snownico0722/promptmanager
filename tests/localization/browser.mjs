...
let browser;
const passed = [];
const record = results => { passed.push(...results); for (const result of results) console.log('PASS ' + result); };
try {
  ...
  record(await page.evaluate(() => runDomChecks()));
  await page.close();
  ...
  record(await log.evaluate(() => runChangelogChecks()));
  await log.close();
  ...
  record(await failure.evaluate(() => runChangelogFailureChecks()));
  await failure.close();
  ...
  for (const name of ['sidepanel/index.html?expanded=1', 'settings.html', 'permissions/permissions.html', 'opd-settings.html']) {
    ...
    try {
      ...
      if (name.startsWith('sidepanel/')) {
        ...
        // Puppeteer click does not wait for the overflow's width/transform transition.
        await ui.waitForFunction(() => {
          const overflow = document.querySelector('.spm-actions-menu-open .spm-prompt-actions-overflow');
          return overflow && getComputedStyle(overflow).maxWidth === '56px'
            && getComputedStyle(overflow).transform === 'matrix(1, 0, 0, 1, 0, 0)';
        });
        ...
        await ui.evaluate(() => OPMI18n.setLanguage('zh-CN'));
        record(['sidepanel copy/edit round trip, literal tags, stable actions and preserved draft']);
      }
      assert.deepEqual(errors, [], `${name}: script errors`);
      record([`${name}: translated real page modules with mocked Chrome APIs`]);
    } finally {
